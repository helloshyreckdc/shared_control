...
int main(int argc, char** argv)
{
  ...
  /******  Passthrough filter, X direction  ******/
  pcl::PassThrough<PointT> xPass;
  xPass.setInputCloud(cloud_down_sampled);
  xPass.setFilterFieldName("x");
  xPass.setFilterLimits(-0.16, 0.12);  //Change according to actual conditions
  xPass.filter(*cloud_pass_through_x);
  ROS_INFO("PointCloud after pass through in x direction: %d data points. ", cloud_pass_through_x->width * cloud_pass_through_x->height);
  ...
  seg.setInputCloud(cloud_pass_through_x);
  seg.segment(*inliers, *coefficients);
  //Create the filtering object
  pcl::ExtractIndices<pcl::PointXYZ> extract;
  extract.setInputCloud(cloud_pass_through_x);
  ...
  /******  Passthrough filter, Y direction  ******/
  pcl::PassThrough<PointT> yPass;
  yPass.setInputCloud(cloud_segmented);
  yPass.setFilterFieldName("y");
  yPass.setFilterLimits(-0.1, 0.12);  //Change according to actual conditions
  yPass.filter(*cloud_pass_through_y);
  ROS_INFO("PointCloud after pass through in y direction: %d data points. ", cloud_pass_through_y->width * cloud_pass_through_y->height);

  /******  Statistical Removal  ******/
  pcl::StatisticalOutlierRemoval<PointT> sor_rm_outlier;
  sor_rm_outlier.setInputCloud(cloud_pass_through_y);
...
//                     0                , 0                , 0                , 1                           ;
  ...
  while(ros::ok())
  {
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "camera_depth_optical_frame", "cube_target"));
